...
namespace Db_Final
{
    public partial class InverterView : Form
    {
        public InverterView()
        {
            InitializeComponent();
        ...
        private void InverterView_Load(object sender, EventArgs e)
        {
            this.WindowState = FormWindowState.Maximized;

            var con = Configuration.getInstance().getConnection();
            SqlCommand cmd = new SqlCommand("Select * from Inverter", con);
        ...
        private void guna2Button8_Click(object sender, EventArgs e)
        {
            if (guna2DataGridView1.SelectedRows.Count > 0)
            {
                ...
                string deleteQuery = "DELETE FROM Inverter WHERE LocationId = @LocationId";
                var con = Configuration.getInstance().getConnection();
